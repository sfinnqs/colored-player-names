...
public class Updater {
	 ...
	 */
	DEFAULT, /**
		  * Don't run a version check, just find the latest update and
		  * download it.
		  */
	NO_VERSION_CHECK, /**
			   * Get information about the version and the download
			   * size, but don't actually download anything.
			   */
	NO_DOWNLOAD
    }
	...
	try {
	    ...
	    fout = new FileOutputStream(
		    new File(this.updateFolder, file.getName()));
		...
		if (this.announce && ((percent % 10) == 0)) {
		    this.plugin.getLogger().info("Downloading update: "
	...
	for (final File dFile : list) {
		...
		for (File cFile : dList) {
			...
			this.fileIOOrError(output, cFile.renameTo(output),
				true);
		    } else {
	...
	if (!result) {
	    this.plugin.getLogger()
		    .severe("The updater could not "
			    + (create ? "create" : "delete") + " file at: "
	...
	if (this.url != null && (this.read() && this.versionCheck())) {
		...
		if (this.versionLink.endsWith(".zip")) {
		    name = this.versionLink
			    .substring(this.versionLink.lastIndexOf("/") + 1);
		}
		this.saveFile(name);
	    } else {
